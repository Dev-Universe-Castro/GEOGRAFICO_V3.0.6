...
@app.route('/api/dossie/pdf/<int:revenda_id>')
@login_required
def generate_dossie_pdf(revenda_id):
    try:
        ...
        # 1) Cabeçalho com Logo da Ferticore
        try:
            logo_path = 'static/images/ferticore-logo-full.png'
        ...
        # Title
        elements.append(Paragraph(f"Dossiê - {revenda.nome}", title_style))
        elements.append(Paragraph(f"CNPJ: {revenda.cnpj}", normal_style))
        elements.append(Spacer(1, 20))

        # Get analysis data
        analysis_data = calculate_revenda_analysis(municipios_codes)

        # 2) Receitas e Despesas - Tabela dos municípios
        elements.append(Paragraph("2. Receitas e Despesas", subtitle_style))

        # Create financial table
        financial_data = []
        financial_data.append(['Município', 'Estado', 'Receita (R$)', 'Despesa (R$)', 'Saldo (R$)'])

        for municipio_data in analysis_data['financialData']['municipios']:
            saldo = municipio_data['receita'] - municipio_data['despesa']
            financial_data.append([
                municipio_data['name'][:20] + '...' if len(municipio_data['name']) > 20 else municipio_data['name'],
                municipio_data['state'],
                f"R$ {municipio_data['receita']:,.0f}",
                f"R$ {municipio_data['despesa']:,.0f}",
                f"R$ {saldo:,.0f}"
            ])
        ...
        # 3) Culturas - Tabela com todas as culturas
        elements.append(Paragraph("3. Culturas", subtitle_style))

        crops_data = []
        crops_data.append(['Cultura', 'Área Total (ha)', 'Municípios', 'Participação (%)'])

        total_area = sum([crop['total_area'] for crop in analysis_data['cropsData']['crops']])
        for crop in analysis_data['cropsData']['crops']:  # All cultures, not limited
            participation = (crop['total_area'] / total_area * 100) if total_area > 0 else 0
            crops_data.append([
                crop['name'][:15] + '...' if len(crop['name']) > 15 else crop['name'],
                f"{crop['total_area']:,.0f}",
                str(crop['municipalities_count']),
                f"{participation:.1f}%"
            ])

        crops_table = Table(crops_data)
        ...
        # Helper function to create section with table and chart
        def create_data_section(title, data_key, analysis_data, unit_label="estabelecimentos"):
            elements.append(Paragraph(f"{title}", subtitle_style))

            # Create table
            table_data = []
            table_data.append(['Categoria', f'Estabelecimentos', 'Participação (%)'])

            section_data = analysis_data[data_key]
            total = sum([cat['total'] for cat in section_data['categories']])

            for category in section_data['categories']:
                participation = (category['total'] / total * 100) if total > 0 else 0
                table_data.append([
                    category['name'][:25] + '...' if len(category['name']) > 25 else category['name'],
                    f"{category['total']:,.0f}",
                    f"{participation:.1f}%"
                ])
            ...
            # Create pie chart
            if len(section_data['categories']) > 0:
                try:
                    # Create matplotlib pie chart
                    fig, ax = plt.subplots(figsize=(8, 6))

                    labels = [cat['name'][:15] + '...' if len(cat['name']) > 15 else cat['name'] for cat in section_data['categories'][:10]]
                    sizes = [cat['total'] for cat in section_data['categories'][:10]]
        ...
        # Helper function for Escolaridade section (uses "pessoas" instead of "estabelecimentos")
        def create_escolaridade_section(title, data_key, analysis_data):
            elements.append(Paragraph(f"{title}", subtitle_style))

            # Create table
            table_data = []
            table_data.append(['Categoria', 'Pessoas', 'Participação (%)'])
        ...
        # 4) Fertilizantes
        create_data_section("4. Fertilizantes", 'fertilizerData', analysis_data)

        # 5) Agrotóxicos
        create_data_section("5. Agrotóxicos", 'agrotoxicoData', analysis_data)

        # 6) Consultoria
        create_data_section("6. Consultoria", 'consultoriaData', analysis_data)

        # 7) Corretivos
        create_data_section("7. Corretivos", 'corretivosData', analysis_data)

        # 8) Escolaridade
        create_escolaridade_section("8. Escolaridade", 'escolaridadeData', analysis_data)
        ...
        response.headers['Content-Disposition'] = f'attachment; filename=dossie_{revenda.nome.replace(" ", "_")}.pdf'
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': f'Erro ao gerar PDF: {str(e)}'})
...
def get_analise_potencial(revenda_id):
    try:
        ...
        analysis_result = analyze_revenda_potential(revenda.get_municipios_list())
...
def get_analise_potencial_vendedor(vendedor_id):
    try:
        ...
        print(f"Analyzing potential for vendedor: {vendedor.nome} (ID: {vendedor_id})")

        # Obter dados de todas as fontes
        analysis_result = analyze_revenda_potential(vendedor.get_municipios_list())
...
def get_revenda_territory_data(revenda_id):
    try:
        ...
        # Fetch revenda data from the database
        revenda = Revenda.query.get_or_404(revenda_id)
    ...
    try:
        ...
        for vendedor in vendedores:
            vendedores_list.append({
                ...
                'cpf': vendedor.cpf,
                'cor': vendedor.cor,
                'municipios': vendedor.get_municipios_list(),
                'municipios_count': len(vendedor.get_municipios_list()),
                'created_at': vendedor.created_at.strftime('%d/%m/%Y')
            })
...
def create_vendedor():
    try:
        ...
        # Validar dados obrigatórios
        if not data.get('nome') or not data.get('email') or not data.get('telefone') or not data.get('cpf'):
            return jsonify({'success': False, 'error': 'Nome, email, telefone e CPF são obrigatórios'}), 400
        ...
        existing_vendedor_email = Vendedor.query.filter_by(email=data['email']).first()
        if existing_vendedor_email:
            return jsonify({'success': False, 'error': 'E-mail já cadastrado'}), 400

        # Verificar se CPF já existe
        existing_vendedor_cpf = Vendedor.query.filter_by(cpf=data['cpf']).first()
        if existing_vendedor_cpf:
        ...
        vendedor.set_municipios_list(data['municipios'])

        print(f"Vendedor created with municipalities: {vendedor.get_municipios_list()}")

        db.session.add(vendedor)
        db.session.commit()

        print(f"Vendedor saved with ID: {vendedor.id}")
...
def update_vendedor(vendedor_id):
    try:
        ...
        # Atualizar campos (CPF não pode ser alterado)
        if 'nome' in data:
            vendedor.nome = data['nome']
        if 'email' in data:
            # Verificar se o novo email já existe (exceto para o próprio vendedor)
            existing_vendedor = Vendedor.query.filter(
                Vendedor.email == data['email'],
                Vendedor.id != vendedor_id
            ).first()
            if existing_vendedor:
                return jsonify({'success': False, 'error': 'E-mail já está sendo usado por outro vendedor'}), 400
            vendedor.email = data['email']
        if 'telefone' in data:
            vendedor.telefone = data['telefone']
        if 'cor' in data:
            vendedor.cor = data['cor']
        if 'municipios' in data:
            vendedor.set_municipios_list(data['municipios'])
...
def get_vendedor_territory_data(vendedor_id):
    try:
        ...
        # Fetch vendedor data from the database
        vendedor = Vendedor.query.get_or_404(vendedor_id)
        print(f"Loading territory data for vendedor: {vendedor.nome} (ID: {vendedor_id})")

        # Obter lista de municípios do vendedor
